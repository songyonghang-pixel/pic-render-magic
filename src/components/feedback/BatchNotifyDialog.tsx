...
export const BatchNotifyDialog = ({ open, count, onClose, onApply }: Props) => {
  ...
  const [aiJudgeValue, setAiJudgeValue] = useState("旁路");
      ...
      <div className="bg-card rounded-md w-full max-w-2xl max-h-[85vh] flex flex-col shadow-xl">
        ...
        <div className="p-5 space-y-4 overflow-auto">
          ...
          <div className="flex gap-2 text-[12.5px] rounded-sm p-2.5 bg-destructive/10 text-destructive">
            <span className="shrink-0">提示：若预警规则开启“按级别分别设置”时，批量修改通知设置将关闭分别设置选项。</span>
          </div>
...
const ToggleRow = ({
  ...
  <div className="flex items-center justify-between rounded-md border border-[hsl(var(--field-border))] bg-card px-4 py-3">
    <div className="flex items-center gap-3">
      <Switch checked={checked} onCheckedChange={onChange} />
      <span className="text-[13px] text-[hsl(var(--label-text))]">{label}</span>
    </div>
    <HoverCard openDelay={100}>
      <HoverCardTrigger asChild>
